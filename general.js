...
function btns_refresh_by_year(year) {
    for (var i = 1; i <= 12; i++) {
        ...
        if (btn != null) {
            if (is_btn_select(btn))
                btn.className = "table_year_om_sel" + is_now(btn);
            else btn.className = "table_year_td" + is_now(btn);

            if (!is_btn_en(btn)) btn.className = "table_year_td_dis";
        }
    }
}
...
function on_mouse_enter(e) {
    ...
    if (
        btn.className != "table_year_om_sel" &&
        btn.className != "table_year_om_sel_new"
    )
...
function create_month(parent, year, month, mounth_text) {
    ...
    if (!is_btn_en(td)) td.className = "table_year_td_dis";
    else td.className = "table_year_td" + is_now(td);
